...
- **Only `.haml-lint.yml` and `.rubocop.yml` are watched.** Changing either re-lints the Haml files you have open. A configuration reached some other way — a file named by `haml.hamlLint.configPath`, or one pulled in by `inherits_from` — is still read on every run, but changing it does not refresh anything on its own until you edit a `.haml` file or run `Haml: Lint File`.
